...
async function sendJsonRpc(port: number, version: string, message: Record<string, unknown>) {
  const response = await fetch(`http://127.0.0.1:${port}`, {
    method: "POST",
    headers: { "content-type": "application/json", "a2a-version": version },
    body: JSON.stringify({ jsonrpc: "2.0", id: `request-${version}`, method: "message/send", params: { message } }),
  });
  return { response, body: (await response.json()) as JsonRpcTask };
}
...
test("A2A v0.3 JSON-RPC compatibility serves legacy kagent", { timeout: 15000 }, async (t) => {
  ...
  const { response, body } = await sendJsonRpc(server.httpPort, "0.3", {
    kind: "message",
    contextId: "kagent-ui-context",
    taskId: "task-1",
    role: "user",
...
test("A2A v1 JSON-RPC preserves caller identity", { timeout: 15000 }, async (t) => {
  const pi = new FakePi();
  const server = await startServer(pi, { ...options(), httpHost: "127.0.0.1", httpPort: 0 });
  t.after(() => server.close());
  assert.ok(server.httpPort !== undefined);
  const { response, body } = await sendJsonRpc(server.httpPort, "1.0", {
    kind: "message",
    messageId: "message-v1",
    contextId: "context-v1",
    taskId: "task-v1",
    role: "user",
    parts: [{ kind: "text", text: "Hello over A2A v1" }],
  });
  assert.equal(response.status, 200);
  assert.equal(body.error, undefined);
  assert.equal(body.result?.contextId, "context-v1");
  assert.deepEqual(pi.prompts, ["Hello over A2A v1"]);
});
